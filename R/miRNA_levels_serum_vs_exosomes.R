##<<<<<<<<<<<<<<<<<<<<<<<<<HEAD
## This script contains the data analysis of miRNA serum levels in a humanized tumor mouse (HTM) model (comparison of miRNA serum levels vs. exosomal miRNA). This proof of principle investigation 
...
## load data
dat <- read_csv("Data/miRNA_serum_vs_exosome.csv")

## convert data to tidy format and wrangle data
dat <- dat %>% 
  gather("miRNA", "expression", -c(mouse, treatment, type)) %>%
...
save_small_plots <- function(data, trt){
  lapply(levels(data$miRNA), function(i){
    ...
    png(paste("Results/", unique(data$mouse), trt, str_remove(i, "\\*+"), "high_exo_ratio", ".png", sep ="_"), units = "in", width = 3, height = 4, res = 600)
    print(p)
    dev.off()
  })
}
...
save_plots <- function(data, trt){
  ...
  p4 <- annotate_figure(p4, top = text_grob(paste(dat_plot$mouse, dat_plot$treatment), 
                                      color = "black", face = "bold", size = 14))
  
  png(paste("Results/", dat_plot$mouse, dat_plot$treatment, "serum_exosome_miRNA", ".png", sep ="_"), units = "in", width = 18, height = 6, res = 600)
  print(p4)
  dev.off()
}
